...
| Web tools | Six pages on the Python backend — `/tools/signer`, `/tools/repo-creator`, `/tools/repo-decoder`, `/tools/app-installer`, `/tools/cert-check`, `/tools/udid` — with matching JSON APIs, 63 tests, and a launcher in Settings → Ecosystem → Web Tools. | The repo creator does not host files; the cert checker never takes a `.p12` or password and reports *unknown* rather than *not revoked*; UDID sessions are in-memory, 15 minutes, unsigned profile. See [WEBTOOLS.md](WEBTOOLS.md). |
...
server/{web_tools,web_chrome,web_signer,repo_creator,repo_decoder,app_installer,cert_inspector,udid_grabber,request_base}.py
server/tests/{test_web_tools,test_signer_decoder_installer}.py
