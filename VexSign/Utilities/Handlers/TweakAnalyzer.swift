...
//  crash, and VexSign can auto-pick a sensible injection path.
//
...
struct TweakAnalysis: Equatable {
	...
	var usesRpath: Bool
	/// Suggested injection settings VexSign can apply (nil for non-injectable types).
...
enum TweakAnalyzer {
	...
	private static func _analyzeMacho(at machoURL: URL, type: TweakFileType, placement: String?, appURL: URL?) -> TweakAnalysis {
		...
		// Substrate is handled via notes, not a "missing" row: VexSign auto-adds ElleKit
		...
		if needsSubstrate {
			if let app = appURL, _appHasSubstrate(app) {
				...
				notes.append(.localized("Hooks via CydiaSubstrate. The app doesn't include it, so VexSign adds ElleKit (a Substrate replacement) automatically when signing."))
			}
		}
		if loads.isEmpty {
	...
	private static func _analyzeDeb(at url: URL, appURL: URL?) async -> TweakAnalysis {
		...
		var notes: [String] = [.localized("VexSign unpacks the .deb and injects each dylib/framework and copies bundles automatically.")]
		if needsSubstrate {
			if let app = appURL, _appHasSubstrate(app) {
				notes.append(.localized("Needs CydiaSubstrate — the app already includes it."))
			} else {
				notes.append(.localized("Needs CydiaSubstrate. The app doesn't include it, so VexSign adds ElleKit automatically when signing."))
			}
		}
