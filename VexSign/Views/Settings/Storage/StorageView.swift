...
extension StorageCategory {
	...
	var explanation: String? {
		...
		case .caches: .localized("Icons and web data VexSign downloaded. Rebuilt as you browse.")
		case .other: .localized("Anything outside VexSign's own folders, including files you added yourself. The app database is locked so it can't be deleted here.")
		default: nil
		}
	}
}
	...
	var body: some View {
		VStack(alignment: .leading, spacing: 10) {
			HStack(alignment: .firstTextBaseline) {
				...
				Text(.localized("used by VexSign"))
					.font(.subheadline)
					.foregroundStyle(.secondary)
			}
